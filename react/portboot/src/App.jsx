...
export default function App() {
  // 컴포넌트가 처음 화면에 나타날 때 한 번 실행되는 코드
  useEffect(() => {
    //mount = 처음 렌더링 될 때
    const handleScroll = () => {
      document.querySelectorAll(".slideanim").forEach((el) => {
        ...
        // 요소의현재화면 상단으로부터의 거리(px)를 구합니다. - 요소가 현재 뷰포트의 어디쯤 있는지를 알 수 있음
        const winHeight = window.innerHeight; //브라우저 높이
        if (pos < winHeight - 100) {
          // 요소의 위치가 화면 아래쪽의 100px 위치에 도달하면 실행
    ...
    // 클린업 컴포넌트가 사라질 때 -> 컴포넌트가 사라질 때 스크롤 이벤트 제거(메모리 누수방지)
