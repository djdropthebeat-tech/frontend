...
useEffect(()=>{
  const handleScroll = () => {
    ...
    // window.pageYOffset: 수직 스크롤 위치
    // window.innerHeight/3 : 화면 높이 1/3 = 화면 상단에서 1/3위치를 감지기준
    sections.forEach((section, index)=>{
    const top = section.offsetTop;  // 현재순회중인 섹션의 시작
    const bottom = top + section.offsetHeight;
    // scrollPosition (감지위치)가 현재 순회중인 섹션의 시작과 끝 사이에 있는 경우

    if(scrollPosition >= top && scrollPosition < bottom){
      setActiveSection(index);
      // 조건이 참이면 해당 섹션의 (index) 값으로 activeSection 상태를 업데이트
    };
  });
};
...
// 컴포넌트 화면에 나타난 후 , 사용자가 스크롤 할 때마다 모든  .section요소를 검사하여
// 화면의 상단 1/3 지점에 걸쳐있는 섹션이 무엇인지 확인하고, 그 섹션의 인덱스를 
    ...
    <div className='app'>
      ...
      <section ref={portfolioRef} className="section section-portfolio">
        <div className="section-content">
          ...
          <p className="fade-in-delay">저의 작업물들을 소개합니다.</p>
