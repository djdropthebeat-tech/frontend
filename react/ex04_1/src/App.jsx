...
export default function App() {
  ...
  // 처음부터 에러를 보여주지 않기 위해(입력하기도 전에 에러가 뜨지 않도록)
  ...
  const validateEmail = (email) => {
    ...
    // /^[^\s@]   : username은 공백이나 @를 포함할 수 없고, 하나 이상의 문자가 존재
    // @[^\s@]+\  : domain도 공백과 @를 제외한 문자로 이루어져야 합니다.
    // \.[^\s@]+$ : extention은 공백과 @를 제외한 문자로 이루어져야 하며, 마침표로 구분
    return emailRegex.test(email);
    // test 정규표현식 메서드 -> 일치하면 true, 일치하지않으면 false
  };
